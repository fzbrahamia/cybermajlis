...
function timeAgo(d: Date, isAR: boolean) {
...
// ── TierBadge ─────────────────────────────────────────────────────────────────
function TierBadge({tier,count,isAR}:{tier:Tier;count:number;isAR:boolean}) {
  if (!tier) return null;
  const t = TIERS[tier];
  return (
...
// ── Certificate ───────────────────────────────────────────────────────────────
function Certificate({displayName,count,date,onClose,isAR}:{displayName:string;count:number;date:string;onClose:()=>void;isAR:boolean}) {
      ...
      <div style={{maxWidth:800,width:"100%"}}>
        <div ref={certRef} style={{background:"linear-gradient(160deg,#0d0407,#1c0810)",border:"2px solid rgba(197,165,126,0.4)",borderRadius:16,padding:"48px 56px",position:"relative",overflow:"hidden",fontFamily:"var(--ui)",boxShadow:"0 0 0 1px rgba(197,165,126,0.1) inset, 0 40px 80px rgba(0,0,0,0.6)"}}>
          ...
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:10,letterSpacing:"0.35em",color:"rgba(197,165,126,0.5)",textTransform:"uppercase",marginBottom:6}}>
              {isAR ? "مجتمع المجلس السيبراني" : "CyberMajlis Community"}
            </div>
            ...
            <div style={{display:"flex",justifyContent:"center",gap:40,marginTop:8}}>
              ...
              <div style={{textAlign:"center"}}>
                ...
                <div style={{fontSize:11,color:"rgba(197,165,126,0.7)",marginTop:2}}>{count} {isAR ? "موثّق" : "verified"}</div>
              </div>
            </div>
          </div>
        </div>
...
// ── Submit Modal ──────────────────────────────────────────────────────────────
function SubmitModal({userId,userTier,approvedCount,onClose,onSubmitted,isAR}:{userId:string|null;userTier:Tier;approvedCount:number;onClose:()=>void;onSubmitted:()=>void;isAR:boolean}) {
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:9998,background:"rgba(0,0,0,0.55)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem"}}>
      <div style={{background:"#faf6f1",borderRadius:24,width:"100%",maxWidth:500,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 32px 80px rgba(62,19,22,0.35)",border:"1px solid rgba(197,165,126,0.2)",position:"relative"}}>
        <div style={{background:"linear-gradient(135deg,#FBF4E8,#F1E8D6)",borderBottom:"1px solid rgba(99,32,36,0.1)",padding:"1.3rem 1.5rem",borderRadius:"24px 24px 0 0",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <button suppressHydrationWarning onClick={onClose} style={{background:"rgba(99,32,36,0.06)",border:"none",color:"#6a4640",width:30,height:30,borderRadius:"50%",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><X size={16} /></button>
          </div>
        </div>
            ...
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.6rem"}}>
                {(Object.entries(TYPES) as [WType,typeof TYPES[WType]][]).map(([k,c])=>(
                  <button suppressHydrationWarning key={k} onClick={()=>{setType(k);setStep("form");}}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"0.9rem"}}>
              ...
              <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
                <div style={{position:"relative",width:38,height:20}}>
                  <input suppressHydrationWarning type="checkbox" checked={form.anon} onChange={e=>setForm(f=>({...f,anon:e.target.checked}))} style={{opacity:0,position:"absolute",inset:0,cursor:"pointer"}}/>
...
export default function CommunityPage() {
  ...
  const userTier = getTier(approvedCount);
  ...
  return(
    <div suppressHydrationWarning style={{minHeight:"100vh",background:"#FDFBF6",fontFamily:"'DM Sans',sans-serif"}}>
        ...
        <main>
          {loading?(
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"1rem"}}>
              {filtered.map(r=><ReportCard key={r.id} r={r} userId={userId} onUpvote={handleUpvote} isAR={isAR}/>)}
            </div>
          )}
        </main>
      </div>

      {showSubmit&&<SubmitModal userId={userId} userTier={userTier} approvedCount={approvedCount} onClose={()=>setShowSubmit(false)} onSubmitted={()=>{loadReports();setApprovedCount(c=>c+1);}} isAR={isAR}/>}
      {showCert&&<Certificate displayName={displayName} count={approvedCount} date={certDate} onClose={()=>setShowCert(false)} isAR={isAR}/>}
    </div>
  );
}
