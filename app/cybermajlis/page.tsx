...
/* Longer and softer than EASE, for the big staged reveals. */
const EASE_SLOW = [0.16, 1, 0.3, 1] as const;

/* Layered depth: contact shadow, mid spread, wide ambient, top highlight. */
const LIFT = [
  "inset 0 1px 0 rgba(255,255,255,.85)",
  "0 1px 2px rgba(58,44,28,.04)",
  "0 10px 24px rgba(99,32,36,.07)",
  "0 30px 60px rgba(99,32,36,.06)",
].join(", ");
...
const item = {
  ...
  show:   { opacity: 1, y: 0, transition: { duration: 0.9, ease: EASE_SLOW } },
};
...
function CommunityMock() {
  const isAR = useLocale() === "ar";
  const reports = [
    { av: "/characters/falcon.jpeg", name: isAR ? "نورة" : "Noora", tag: isAR ? "احتيال هاتفي" : "SCAM CALL", c: "#e0796a", en: "Caller posing as the bank asked for my OTP. Reported the number.", ar: "متصل يدّعي أنه البنك طلب رمز التحقق. أبلغت عن الرقم.", warned: 38 },
    { av: "/characters/oryx.jpeg", name: isAR ? "خالد" : "Khalid", tag: isAR ? "تصيّد" : "PHISHING", c: "#d9a14a", en: "Fake 'Ooredoo bill' SMS with a payment link. Reported so others are warned.", ar: "رسالة 'فاتورة Ooredoo' مزيفة برابط دفع. أبلغت عنها لتحذير الآخرين.", warned: 52 },
  ];
  return (
    <>
        ...
        {reports.map((r, i) => (
          <div key={i} style={{ padding: 11, borderRadius: 12, background: "rgba(255,255,255,.04)", border: "1px solid rgba(197,165,126,.14)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}>
              <img src={r.av} alt="" loading="lazy" style={{ width: 26, height: 26, borderRadius: "50%", objectFit: "cover", border: "1px solid rgba(197,165,126,.4)" }} />
...
function FeatureSection({ f, index, isAR, onCTA }: { f: Feature; index: number; isAR: boolean; onCTA: () => void }) {
  ...
  const btn: React.CSSProperties = { color: C.cream, background: "linear-gradient(135deg,#7a1e22,#8B2635)", boxShadow: "0 1px 2px rgba(58,44,28,.12), 0 10px 24px rgba(99,32,36,.20)" };
        ...
        return (
          ...
          >
            ...
            <span style={{
              ...
              boxShadow: on ? "0 0 0 4px rgba(99,32,36,.12), 0 8px 20px rgba(99,32,36,.14)" : LIFT, transition: "all .3s ease",
            }} />
...
export default function HomePage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.paper, fontFamily: crimson, color: C.heading, position: "relative" }}>
      ...
      {/* Paper grain, so the flat cream reads as stock rather than a fill.
          Sections paint their own opaque fills at zIndex 1, so this has to sit
          above them at 2. It stays below the nav (50) and the side rail (40).
          Kept light because the closing section is dark and multiply muddies it. */}
      <div aria-hidden style={{
        position: "fixed", inset: 0, zIndex: 2, pointerEvents: "none",
        backgroundImage: GRAIN, opacity: 0.22, mixBlendMode: "multiply",
      }} />
        ...
        <div style={{ width: "100%", display: "flex", gap: "clamp(40px,5vw,80px)", alignItems: "center", flexWrap: "wrap" }}>
          ...
          <motion.div variants={container} initial="hidden" animate="show" style={{ flex: "1 1 440px", maxWidth: 580 }}>
            ...
            <motion.h1 variants={item} style={{
              ...
              lineHeight: 1.0, letterSpacing: "-0.03em", margin: "22px 0 6px",
            }}>
            ...
            <motion.div variants={item} style={{ display: "flex", gap: 14, marginTop: 30, flexWrap: "wrap" }}>
              <button
                ...
                style={{
                  ...
                  boxShadow: "0 1px 2px rgba(58,44,28,.14), 0 10px 26px rgba(99,32,36,.32)", transition: "all .25s ease",
          ...
          >
            ...
            <div onClick={handleVideoClick} style={{
              ...
              boxShadow: "0 2px 6px rgba(58,44,28,.08), 0 20px 44px rgba(99,32,36,.16), 0 48px 90px rgba(99,32,36,.12), 0 0 0 1px rgba(197,165,126,.2)",
            }}>
