...
export function useElderLang(): ["en" | "ar", (l: "en" | "ar") => void] {
  ...
  // Open seniors mode in the same language the user is already using on the main
  // site (the shared `locale` cookie). Falls back to any saved elder preference.
  useEffect(() => {
  ...
  const setLang = (l: "en" | "ar") => {
    // Keep seniors mode and the main site in sync via the shared locale cookie.
